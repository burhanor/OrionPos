...
namespace OrionPos.Dto.DirectoryDto
{
    public class UpdateDirectoryDto
    {
        ...
        [RegularExpression(@"[a-zA-ZğüşıöçİĞÜŞÖÇ\s]*", ErrorMessage = "{0} sadece harf içerebilir")]

        public string FirstName { get; set; } = string.Empty;
        [DisplayName("Soyad")]
        [Required(ErrorMessage = "{0} boş olamaz")]
        [StringLength(100, ErrorMessage = "{0} {1} karakterden fazla olamaz")]
        [RegularExpression(@"[a-zA-ZğüışöçİĞÜŞÖÇ\s]*", ErrorMessage = "{0} sadece harf içerebilir")]
